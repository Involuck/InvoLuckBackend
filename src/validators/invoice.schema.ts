...
export const updateInvoiceSchema = createInvoiceSchema
  .partial()
  .omit({ clientId: true }) // Cannot change client after creation
...
export const updateInvoiceStatusSchema = z.object({
  ...
  paidDate: z
    .string()
    .datetime('Invalid paid date format')
    .or(z.date())
    .optional(),
  paymentMethod: z
...
export const invoiceParamsSchema = z.object({
  id: z
    .string()
    .regex(/^[a-fA-F0-9]{24}$/, 'Invalid invoice ID format'),
});
...
export const sendInvoiceSchema = z.object({
  ...
  cc: z
    .array(z.string().email())
    .max(5, 'Maximum 5 CC recipients allowed')
    .optional(),
...
export const invoiceAnalyticsSchema = z.object({
  ...
  groupBy: z
    .enum(['day', 'week', 'month', 'client', 'status'])
    .default('month'),
});
