...
export function setupDevelopmentRoutes(app) {
        ...
        try {
            if (!emailService.isConfigured()) {
                return res.status(503).json({
                    ...
                    message: 'Email service is not available. Check your RESEND_API_KEY and ADMIN_EMAIL configuration.',
    ...
    app.get('/throttle-status', (req, res) => {
        try {
            ...
            const criticalThrottle = emailService.getThrottleStatus('critical', { error: { name: 'TestError', message: 'Test' } });
            ...
            return res.json({
                ...
                throttleStatus: {
                    testEmail: {
                        ...
                        nextAllowedTime: testThrottle.throttled ? new Date(Date.now() + testThrottle.waitMs).toISOString() : null
                    },
                    critical: {
                        throttled: criticalThrottle.throttled,
                        waitTimeSeconds: criticalThrottle.throttled ? Math.ceil(criticalThrottle.waitMs / 1000) : 0,
                        nextAllowedTime: criticalThrottle.throttled ? new Date(Date.now() + criticalThrottle.waitMs).toISOString() : null
                    }
                },
                throttleSettings: {
